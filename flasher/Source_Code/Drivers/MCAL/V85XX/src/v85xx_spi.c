...
void SPI_Init(SPI_TypeDef *SPIx, SPI_InitType *InitStruct)
{
  ...
  assert_param(IS_SPI_SPH(InitStruct->SPH));
  assert_param(IS_SPI_SPO(InitStruct->SPO));
  assert_param(IS_SPI_CLKDIV(InitStruct->ClockDivision));
  assert_param(IS_SPI_CSN(InitStruct->CSNSoft));
  assert_param(IS_SPI_SWAP(InitStruct->SWAP));
...
void SPI_INTConfig(SPI_TypeDef *SPIx, uint32_t INTMask, uint32_t NewState)
{
  ...
  /* Check parameters */
  assert_param(IS_SPI_ALL_INSTANCE(SPIx)); 
  assert_param(IS_SPI_INT(INTMask));
  assert_param(IS_FUNCTIONAL_STATE(NewState));
...
uint8_t SPI_GetStatus(SPI_TypeDef *SPIx, uint32_t Status)
{
  /* Check parameters */
  assert_param(IS_SPI_ALL_INSTANCE(SPIx)); 
  assert_param(IS_SPI_STSR(Status));  
...
void SPI_ClearStatus(SPI_TypeDef *SPIx, uint32_t Status)
{
  /* Check parameters */
  assert_param(IS_SPI_ALL_INSTANCE(SPIx)); 
  assert_param(IS_SPI_STSC(Status));
...
void SPI_TransmitFIFOLevelConfig(SPI_TypeDef *SPIx, uint32_t FIFOLevel)
{
  ...
  /* Check parameters */
  assert_param(IS_SPI_ALL_INSTANCE(SPIx)); 
  assert_param(IS_SPI_TXFLEV(FIFOLevel));
...
void SPI_ReceiveFIFOLevelConfig(SPI_TypeDef *SPIx, uint32_t FIFOLevel)
{
  ...
  /* Check parameters */
  assert_param(IS_SPI_ALL_INSTANCE(SPIx)); 
  assert_param(IS_SPI_RXFLEV(FIFOLevel));
