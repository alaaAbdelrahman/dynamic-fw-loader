...
void LCD_Init(LCD_InitType *InitStruct)
{
  ...
  /* Check parameters */
  assert_param(IS_LCD_TYPE(InitStruct->Type));
  assert_param(IS_LCD_DRV(InitStruct->Drv));
  assert_param(IS_LCD_FRQ(InitStruct->FRQ));
  assert_param(IS_LCD_SWPR(InitStruct->SWPR));
  assert_param(IS_LCD_FBMODE(InitStruct->FBMODE));
  assert_param(IS_LCD_BKFILL(InitStruct->BKFILL));
 ...
 /* Reset Data Array so all segments will be blanck */
...
void LCD_StructInit(LCD_InitType *LCD_InitStruct)
{
  ...
  LCD_InitStruct->Drv = LCD_DRV_150;
  /* Initialize the FBMODE member */  
  LCD_InitStruct->FBMODE = LCD_FBMODE_BUFA;
  /* Initialize the FRQ member */  
  LCD_InitStruct->FRQ = LCD_FRQ_256H;//LCD_FRQ_512H;
  /* Initialize the SWPR member */  
  LCD_InitStruct->SWPR = 0;
  /* Initialize the Type member */  
  LCD_InitStruct->Type = LCD_TYPE_8COM;
}
void LCD_StructInit_test(LCD_InitType *LCD_InitStruct)
{
  /*--------------- Reset LCD init structure parameters values ---------------*/
  /* Initialize the BKFILL member */
  LCD_InitStruct->BKFILL = LCD_BKFILL_0;
  /* Initialize the Drv member */
  LCD_InitStruct->Drv = LCD_DRV_300;//LCD_DRV_150;
  /* Initialize the FBMODE member */
  LCD_InitStruct->FBMODE = LCD_FBMODE_BUFA;
  /* Initialize the FRQ member */
  LCD_InitStruct->FRQ =LCD_FRQ_64H;// LCD_FRQ_256H;//LCD_FRQ_512H;
  /* Initialize the SWPR member */
  LCD_InitStruct->SWPR = 0;
  /* Initialize the Type member */
  LCD_InitStruct->Type = LCD_TYPE_8COM;
...
void LCD_BiasModeConfig(uint32_t BiasSelection)
{
  ...
  //assert_param(IS_LCD_BMODE(BiasSelection));
...
uint32_t LCD_VoltageConfig(uint32_t VLCDSelection)
{
  ...
  assert_param(IS_LCD_VLCD(VLCDSelection));
