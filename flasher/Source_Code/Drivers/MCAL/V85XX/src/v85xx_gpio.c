...
void GPIOBToF_Init(GPIO_TypeDef *GPIOx, GPIO_InitType *InitStruct)
{
  ...
  assert_param(IS_GPIO_PIN(InitStruct->GPIO_Pin));
  assert_param(IS_GPIO_MODE(InitStruct->GPIO_Mode));
  /*
  if (GPIOx == GPIOE)
...
void GPIOA_Init(GPIOA_TypeDef *GPIOx, GPIO_InitType *InitStruct)
{
  ...
  /* Check parameters */
  assert_param(IS_PMUIO_ALL_INSTANCE(GPIOx));
  assert_param(IS_GPIO_PIN(InitStruct->GPIO_Pin));
  assert_param(IS_GPIO_MODE(InitStruct->GPIO_Mode)); 
...
uint8_t GPIOBToF_ReadInputDataBit(GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin)
{
  ...
  /* Check parameters */
  assert_param(IS_GPIO_ALL_INSTANCE(GPIOx));
  assert_param(IS_GPIO_PINR(GPIO_Pin));
...
void GPIOBToF_WriteBit(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin, uint8_t val)
{
  ...
  assert_param(IS_GPIO_ALL_INSTANCE(GPIOx));
  assert_param(IS_GPIO_PIN(GPIO_Pin)); 
  assert_param(IS_GPIO_BITVAL(val));
...
void GPIOBToF_AFConfig(GPIO_TypeDef* GPIOx, uint32_t GPIO_AFx, uint8_t NewState)
{
  ...
  /* Check parameters */
  assert_param(IS_GPIOAF_ALL_INSTANCE(GPIOx));
  assert_param(IS_GPIO_GPIOAF(GPIO_AFx));
...
{
  /* Check parameters */
  assert_param(IS_GPIO_PMUIOAF(PMUIO_AFx));
  assert_param(IS_FUNCTIONAL_STATE(NewState));
...
void GPIO_PLLDIV_Config(uint32_t Divider)
{
  ...
  /* Check parameters */
  assert_param(IS_GPIO_PLLDIV(Divider));
