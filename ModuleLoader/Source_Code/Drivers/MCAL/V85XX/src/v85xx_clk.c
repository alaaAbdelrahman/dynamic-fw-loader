...
void CLK_ClockConfig(CLK_InitTypeDef *CLK_ClkInitStruct)
{
  ...
  assert_param(IS_CLK_TYPE(CLK_ClkInitStruct->ClockType));
  ...
  {
    assert_param(IS_CLK_XTALHSTA(CLK_ClkInitStruct->XTALH.State));
  ...
  {
    assert_param(IS_CLK_PLLLSRC(CLK_ClkInitStruct->PLLL.Source));
    assert_param(IS_CLK_PLLLSTA(CLK_ClkInitStruct->PLLL.State));
    assert_param(IS_CLK_PLLLFRQ(CLK_ClkInitStruct->PLLL.Frequency));
  ...
  {
    assert_param(IS_CLK_PLLHSRC(CLK_ClkInitStruct->PLLH.Source));
    assert_param(IS_CLK_PLLHSTA(CLK_ClkInitStruct->PLLH.State));
    assert_param(IS_CLK_PLLHFRQ(CLK_ClkInitStruct->PLLH.Frequency));
  ...
  {
    assert_param(IS_CLK_RTCSRC(CLK_ClkInitStruct->RTCCLK.Source));
    assert_param(IS_CLK_RTCDIV(CLK_ClkInitStruct->RTCCLK.Divider));
  ...
  {
    assert_param(IS_CLK_AHBSRC(CLK_ClkInitStruct->AHBSource));
...
#endif
	  timeout=0;
      while (!(ANA->COMPOUT & ANA_COMPOUT_LOCKH)&&((timeout++) <5000))//todo make timeout
      {
...
#endif
	  timeout=0;
      while (!(ANA->COMPOUT & ANA_COMPOUT_LOCKL)&&((timeout++) <5000))//todo make timeout
      {
#ifdef DEBUG_HANG_LOOPING
      uart_debug_byte ('~',5);
#endif
  ...
  {
    assert_param(IS_CLK_HCLKDIV(CLK_ClkInitStruct->HCLK.Divider));
  ...
  {
    assert_param(IS_CLK_PCLKDIV(CLK_ClkInitStruct->PCLK.Divider));
...
{
  /* Check parameters */
  assert_param(IS_CLK_AHBPERIPHRAL(Periphral));
...
{
  /* Check parameters */
  assert_param(IS_CLK_APBPERIPHRAL(Periphral));
  assert_param(IS_FUNCTIONAL_STATE(NewState));
...
{
  /* Check parameters */
  assert_param(IS_CLK_PLLLOCK(PLLStatus));
