...
void DMA_Init(DMA_InitType *InitStruct, uint32_t Channel)
{
  ...
  /* Check parameters */
  assert_param(IS_DMA_CHANNEL(Channel));
  assert_param(IS_DMA_CONTMOD(InitStruct->ContMode));
  assert_param(IS_DMA_TRANSMOD(InitStruct->TransMode));
  assert_param(IS_DMA_REQSRC(InitStruct->ReqSrc));
  assert_param(IS_DMA_DESTADDRMOD(InitStruct->DestAddrMode));
  assert_param(IS_DMA_SRCADDRMOD(InitStruct->SrcAddrMode));
  assert_param(IS_DMA_TRANSSIZE(InitStruct->TransSize));
  if (InitStruct->TransSize == DMA_TRANSSIZE_HWORD)
  {
    assert_param(IS_DMA_ALIGNEDADDR_HWORD(InitStruct->SrcAddr));
    assert_param(IS_DMA_ALIGNEDADDR_HWORD(InitStruct->DestAddr));
  }
  if (InitStruct->TransSize == DMA_TRANSSIZE_WORD)
  {
    assert_param(IS_DMA_ALIGNEDADDR_WORD(InitStruct->SrcAddr));
    assert_param(IS_DMA_ALIGNEDADDR_WORD(InitStruct->DestAddr));    
  }
...
void DMA_AESInit(DMA_AESInitType *InitStruct)
{
  ...
  /* Check parameters */
  assert_param(IS_DMA_AESMOD(InitStruct->Mode)); 
  assert_param(IS_DMA_AESDIR(InitStruct->Direction)); 
...
{
  /* Check parameters */
  assert_param(IS_DMA_INT(INTMask)); 
...
{
  /* Check parameters */
  assert_param(IS_DMA_INTFLAGR(INTMask)); 
...
{
  /* Check parameters */
  assert_param(IS_DMA_INTFLAGC(INTMask));
