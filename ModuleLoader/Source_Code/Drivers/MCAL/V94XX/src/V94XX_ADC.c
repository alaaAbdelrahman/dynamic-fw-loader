...
uint32_t ADC_CalculateVoltage(uint32_t Mode, int16_t adc_data, float *Voltage)
{
  ...
  {
    if ((Mode&0xFUL) > 2UL) /* VDD or BAT2 channel */
    {
...
/**
  ...
  * @note   This function costs about 170us when SystemClock is 26M(Hz).
  ...
  * @param  [in]Division
            ...
            [out]CalResults.BAT2Voltage   is ignored
...
uint32_t ADC_GetBAT1Voltage_Fast(uint32_t Division, ADC_CalResType *CalResults)
{
  ...
  {
    CalResults->BAT1Voltage = (0.00014107*(float)data - 0.00699515) + Vbatcap;
  }
  else
  {
    CalResults->BAT1Voltage = (0.00015392*(float)data + 0.06667986) + Vbatres;
  }
...
/**
  ...
  * @note   This function costs about 3.3ms when SystemClock is 26M(Hz).
...
/**
  * @brief  Get BAT2 Voltage(takes 244us).
  * @note   This function takes about 244us when SystemClock is 26M(Hz).
  *         ADC data refresh time is 117us.
  * @note   This function will release ADC resource(write ADC registers with their
  *         default reset values).
  * @note   ADC configurarion:
  *             - Trigger mode:       manual mode
  *             - Conversion mode:    single channel mode(BAT2 channel 2)
  *             - ADC clock:          3.2M
  *             - Skip samples:       Skip 2 samples
  *             - Down sampling rate: 1/64
  * @param  [in]Division
            ...
            [out]CalResults.BAT2Voltage   The value of BAT2 Voltage
            [out]CalResults.Temperature     is ignored
  * @retval 1  NVR BAT-offset or ADCBGP-gain information checksum error.
            0  Function successed.
  */
uint32_t ADC_GetBAT2Voltage_Fast(uint32_t Division, ADC_CalResType *CalResults)
{
  float Vbatcap;
  float Vbatres;
  ...
  if (Division & ADC_BAT_CAPDIV)
  {
    CalResults->BAT2Voltage =( (0.00014107*(float)data  - 0.00699515) + Vbatcap ) *100;
    CalResults->BAT2Voltage *= 100;
  }
  else
  {
    CalResults->BAT2Voltage = (0.00015392*(float)data  + 0.06667986) + Vbatres;
  }
  /* ADC resource release */
  ADC_DeInit();
...
/**
  * @brief  Get BAT2 Voltage(takes 3.3ms).
  * @note   This function takes about 3.3ms when SystemClock is 26M(Hz).
  *         ADC data refresh time is about 3.2ms.
  * @note   This function will release ADC resource(write ADC registers with their
  *         default reset values).
  * @note   ADC configurarion:
  ...
  * @retval 1  NVR BAT-offset information checksum error.
            0  Function successed.
  */
uint32_t ADC_GetBAT2Voltage_Normal(uint32_t Division, ADC_CalResType *CalResults)
{
  float Vbatcap;
  float Vbatres;
  NVR_BATMEARES BAT_OffsetInfo;
  ...
  if (Division & ADC_BAT_CAPDIV)
  {
    CalResults->BAT2Voltage = (0.00014107*(float)data  - 0.00699515) + Vbatcap;
  }
  else
  {
    CalResults->BAT2Voltage = (0.00015392*(float)data  + 0.06667986) + Vbatres;
...
/**
  ...
  * @note   This function costs about 6.5ms when SystemClock is 26M(Hz).
  *         ADC data refresh time is about 3.2ms.
  * @note   This function will release ADC resource(write ADC registers with their
  *         default reset values).
  * @note   ADC configurarion:
...
{
  timeout_adc = 0;
  while (ANA->ADCCTRL & ANA_ADCCTRL_MTRIG &&((timeout_adc++) <5000));   //A_E timeout waiting 0
...
{
  timeout_adc = 0;
  while (ANA->ADCCTRL & ANA_ADCCTRL_MTRIG &&((timeout_adc++) <5000)) //A_E timeout waiting 3598
  {
  }
}
