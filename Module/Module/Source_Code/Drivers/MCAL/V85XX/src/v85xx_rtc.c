...
#endif
  while ((RTC->CE & RTC_CE_BSY)&&(timeout++ <200)) //TTODO
  {
  }
#ifdef DEBUG_HANG_LOOPING
...
void RTC_WriteRegisters(uint32_t StartAddr, const uint32_t *wBuffer, uint8_t Len)
{
  ...
  /* Parameter check */
  assert_param(IS_RTC_REGOP_STARTADDR(StartAddr));
...
{
  /* Parameter check */
  assert_param(IS_RTC_TIME_YEAR(sTime->Year));
  assert_param(IS_RTC_TIME_MONTH(sTime->Month));
  assert_param(IS_RTC_TIME_DATE(sTime->Date)); 
  assert_param(IS_RTC_TIME_WEEKDAY(sTime->WeekDay)); 
  assert_param(IS_RTC_TIME_HOURS(sTime->Hours)); 
  assert_param(IS_RTC_TIME_MINS(sTime->Minutes)); 
  assert_param(IS_RTC_TIME_SECS(sTime->Seconds)); 
...
void RTC_INTConfig(uint32_t INTMask, uint32_t NewState)
{
  ...
  /* Parameter check */
  assert_param(IS_RTC_INT(INTMask));  
  assert_param(IS_FUNCTIONAL_STATE(NewState));
...
{
  /* Parameter check */
  assert_param(IS_RTC_INTFLAGR(FlagMask));    
...
{
  /* Parameter check */
  assert_param(IS_RTC_INTFLAGC(FlagMask));
...
{
  /* Parameter check */
  assert_param(IS_RTC_WKUSEC_PERIOD(nPeriod));  
...
{
  /* Parameter check */
  assert_param(IS_RTC_WKUMIN_PERIOD(nPeriod)); 
...
{
  /* Parameter check */
  assert_param(IS_RTC_WKUHOUR_PERIOD(nPeriod));
...
{
  /* Parameter check */
  assert_param(IS_RTC_WKUCNT_PERIOD(nClock));
  assert_param(IS_RTC_WKUCNT_CNTSEL(CNTCLK));
...
void RTC_PrescalerConfig(uint32_t Prescaler)
{
  ...
  /* Parameter check */
  assert_param(IS_RTC_CLKDIV(Prescaler));  
