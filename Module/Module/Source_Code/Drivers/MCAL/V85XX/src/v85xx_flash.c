...
void FLASH_Init(uint32_t CSMode)
{
  ...
  /* Check parameters */
  assert_param(IS_FLASH_CSMODE(CSMode));  
...
void FLASH_INTConfig(uint32_t IntMask, uint32_t NewState)
{
  ...
  /* Check parameters */
  assert_param(IS_FLASH_INT(IntMask));
...
{
  /* Check parameters */
  assert_param(IS_FLASH_ADDRESS(SectorAddr));
...
void FLASH_ProgramWord(uint32_t Addr, uint32_t *WordBuffer, uint32_t Length)
{
  ...
  /* Check parameters */
  assert_param(IS_FLASH_ADRRW(Addr));
  ...
  for (i=0; i<Length; i++)
  {
    ...
    while (FLASH->STS != 1 && (counter++ < 10000));
  }
...
void FLASH_ProgramHWord(uint32_t Addr, uint16_t *HWordBuffer, uint32_t Length)
{
  ...
  /* Check parameters */
  assert_param(IS_FLASH_ADRRHW(Addr));
...
{
  /* Check parameters */
  assert_param(IS_FLASH_CHECKSUMADDR(AddrStart,AddrEnd));
