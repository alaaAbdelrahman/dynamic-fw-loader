...
static void I2C_ClearBus(uint16_t sla);
...
static void I2C_SendStart(void)
{
  ...
  while ((I2C_GetINTStatus() == 0)&&(timeout++ <1000));
...
static void I2C_SendByte(uint8_t dat)
{
  ...
  I2C_SendData(dat);
   //vDelay_SW_RFID(300);
  I2C_ClearINTStatus();
  while (I2C_GetINTStatus() == 0 && ++counter < 500); 
}
...
static uint8_t I2C_ReceiveByte(void)
{
  uint32_t counter = 0;
  I2C_ClearINTStatus();
  while (I2C_GetINTStatus() == 0&&(counter++ <100)); //TTODO
...
static void I2C_ClearBus(uint16_t sla)
{
  ...
  }while ((I2C_GetStatusCode() !=0x18)&&++time_out<1000);
  I2C_SendStop();                 //stop
}
...
void I2C_Init(I2C_InitType *InitStruct)
{
  ...
  /* Check parameters */
  assert_param(IS_I2C_GC(InitStruct->GeneralCallAck));
  assert_param(IS_I2C_AA(InitStruct->AssertAcknowledge));
  assert_param(IS_I2C_CLKSRC(InitStruct->ClockSource));
  /* i2c pins configure*/
  GPIO_InitType GPIO_InitStruct;
  GPIO_InitStruct.GPIO_Mode = GPIO_Mode_FORBIDDEN;
  GPIO_InitStruct.GPIO_Pin = I2C_DTA_PIN|I2C_CLK_PIN;
  GPIOBToF_Init(I2C_PORT, &GPIO_InitStruct);
...
uint16_t I2C_MasterReadBytes(I2C_WRType *InitStruct)
{
  ...
  /* Check parameters */ 
  assert_param(I2C_SUBADDR_TYPE(InitStruct->SubAddrType));
...
uint16_t I2C_MasterWriteBytes(I2C_WRType *InitStruct)
{
  ...
  for (i=0; i<(InitStruct->Length); i++)
  {
    ...
    {   
      I2C_SendStop();
      I2C_ClearBus(InitStruct->SlaveAddr);
      I2C_SendStart();                //start
      ret_val = I2C_CheckState(0x08);
      if (ret_val)    return ret_val;
      /* WriteAddr: 1 byte */
  ...
  I2C_SendStop();
  if(InitStruct->device_type == EEPROM_I2C)
    I2C_ClearBus(InitStruct->SlaveAddr);
  return 0;    
}
