...
/*----------------------------------- P4 -------------------------------------*/
  ...
  else
  {
    ...
    {
      /* Get information */
      RTCTempData->RTCTempP4 = (int16_t)(*NVR_RTC2_P4);
      RTC->ACP4 = *NVR_RTC2_P4;      
    }
    else
    {
...
{
  uint32_t vdd_data, dvcc_data, bgp_data, rcl_data, rch_data;
  uint32_t checksum;
  
  vdd_data = *NVR_VDD_MEA1;
  dvcc_data = *NVR_DVCC_MEA1;
  bgp_data   = *NVR_ADCBGP_MEA1;
  rcl_data   = *NVR_RCL_MEA1;
  rch_data   = *NVR_RCH_MEA1;
  /* Calculate checksum1 */
  checksum = ~(vdd_data + dvcc_data + bgp_data + rcl_data + rch_data);
  if (checksum == (*NVR_PWR_CHECKSUM1))
  {
    MEAResult->VDDMEAResult = vdd_data;
    MEAResult->DVCCMEAResult = dvcc_data;
    MEAResult->ADCBGPMEAResult   = bgp_data;
    MEAResult->RCLMEAResult   = rcl_data;
    MEAResult->RCHMEAResult   = rch_data;
    return 0;
  }
  
  vdd_data = *NVR_VDD_MEA2;
  dvcc_data = *NVR_DVCC_MEA2;
  bgp_data   = *NVR_ADCBGP_MEA2;
